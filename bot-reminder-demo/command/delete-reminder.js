...
module.exports = async (message, ctx) => {
  ...
  if (ctx.request === Constants.COMMAND_DELETE) {
    if (reminderKeys.length >= 1) {
      ...
      const reminderPrint = await Promise.all(reminderKeys.map(async key => {
        ...
        const reminder = Object.setPrototypeOf(JSON.parse(await hget(keyHsetReminderObj, key)), Reminder.prototype)

        // Return printable list entry
        return `\n${reminderKeys.indexOf(key) + 1}) ${reminder.getName()} (${reminder.getSchedule()})`
      }))
  ...
  if (ctx.request === Constants.STATE_REMINDER_DELETE_SELECT) {
    ...
    const reminder = Object.setPrototypeOf(JSON.parse(await hget(keyHsetReminderObj, reminderKey)), Reminder.prototype)

    // Delete field in hashset
    const deleted = await hdel(keyHsetReminderObj, reminderKey)

    if (deleted === 1) {
      await cronManager.stopCronJob(chatId, reminder)
